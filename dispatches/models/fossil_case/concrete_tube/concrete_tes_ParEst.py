...
##############################################################################
"""
Routine for parameter estimation of heat transfer coefficients for Concrete TES
units.
...
from concrete_tes_concrete_side import add_1D_heat_transfer_equation
...
# df_U = pd.DataFrame(index=[i for i in range(number_samples)], columns=['HT_coefficient'])
df_U = pd.DataFrame(index=multi_index, columns=list(range(1,segments+1)))
solution_status = pd.DataFrame(index=[i for i in range(number_samples)], columns=['Surrogate','Unit init.', 'Square problem1', 'Square problem2', 'Param. Est.','Obj. function'])
...
# for s in [0]:
    ...
    for t in m.fs.time_periods:
        ...
        m.fs.constant_htc.fix(df_data.loc[s,('U_tes_sim')][0])
    ...
    try:
        ...
        solution_status.loc[s,'Square problem2'] = res.solver.termination_condition

    except:
        pass

    # **************************************************************************
    #     FREEING HEAT TRANSFER COEFFICIENTS AND ADDING EQUALITY CONSTRAINTS
    # **************************************************************************
    for t in m.fs.time_periods:
        m.fs.unit[t].tube_heat_transfer_coefficient.unfix()
        m.fs.unit[t].tube_heat_transfer_coefficient.setlb(20)
        m.fs.unit[t].tube_heat_transfer_coefficient.setub(400)

        # Adding equality constraints to ensure that U is the same for all the segments
        def eq_ht_coefficient_rule(b,t,s):
            return  b.tube_heat_transfer_coefficient[(t,s)] == m.fs.constant_htc

        if m.fs.unit[t].find_component('eq_heat_transfer_coefficient'):
            m.fs.unit[t].eq_heat_transfer_coefficient.activate()
        else:
            m.fs.unit[t].eq_heat_transfer_coefficient = Constraint(m.fs.unit[t].temperature_wall_index, rule=eq_ht_coefficient_rule)

    def least_square_error_rule(b):
        c = b.unit[m.fs.time_periods.last()]
        # ************* T_concrete *************
        # return b.error == sum((b.unit.temperature_wall[b.unit.temperature_wall_index.ordered_data()[i-1]] - T_concrete_end[i-1])**2
        #                             for i in b.unit.segments_set)

        # ************* T_fluid *************
        # return b.error == sum((b.unit.tube.properties[b.unit.temperature_wall_index.ordered_data()[i-1]].temperature -T_fluid_end[i-1])**2
        #                             for i in b.unit.segments_set) +\
        #                                 (b.unit.temperature_wall[b.unit.temperature_wall_index.ordered_data()[20-1]] - T_concrete_end[20-1])**2 +\
        #                                     (b.unit.temperature_wall[b.unit.temperature_wall_index.ordered_data()[15-1]] - T_concrete_end[15-1])**2 +\
        #                                     (b.unit.temperature_wall[b.unit.temperature_wall_index.ordered_data()[1-1]] - T_concrete_end[1-1])**2

        # ************* T_concrete + T_fluid *************
        return b.error == sum((c.temperature_wall[c.temperature_wall_index.ordered_data()[i-1]] - T_concrete_end[i-1])**2 +
                            (c.tube.properties[c.temperature_wall_index.ordered_data()[i-1]].temperature -T_fluid_end[i-1])**2
                                    for i in c.segments_set)

    if m.fs.find_component('error_constraint'):
        m.fs.del_component('error_constraint')
        m.fs.error_constraint = Constraint(rule=least_square_error_rule)

    else:
        m.fs.error_constraint = Constraint(rule=least_square_error_rule)

    try:
        m.fs.constant_htc.unfix()
        res = solver.solve(m.fs, tee=True,symbolic_solver_labels=True)
        log_close_to_bounds(m.fs)
        log_infeasible_constraints(m.fs)
        solution_status.loc[s, 'Param. Est.'] = res.solver.termination_condition
    ...
    for t in m.fs.unit:
        ...
        m.fs.unit[t].eq_heat_transfer_coefficient.deactivate()
        if t!=m.fs.time_periods.first():
            for i in m.fs.unit[t].segments_set:
                eq_link_temp = getattr(m.fs, 'eq_link_temp_block{}_segment_{}'.format(t,i))
                eq_link_temp.deactivate()

    m.fs.error_constraint.deactivate()
        # m.fs.objective.deactivate()

fname = "Case 3 Multiperiod Concrete TES Constant Us.xlsx"
