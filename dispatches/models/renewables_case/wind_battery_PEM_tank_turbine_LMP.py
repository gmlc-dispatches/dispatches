...
def wind_battery_pem_tank_turb_model(wind_capacity_factor, verbose, times=None):
    m = create_model(fixed_wind_mw, pem_bar, fixed_batt_mw, None, fixed_tank_size, h2_turb_bar,
                     wind_capacity_factor, verbose)
...
def wind_battery_pem_tank_turb_mp_block(cap_factor, verbose):
    global pyo_model
    if pyo_model is None:
        pyo_model = wind_battery_pem_tank_turb_model(cap_factor, verbose)
    m = pyo_model.clone()
    m.fs.windpower.config.resource_probability_density = cap_factor['resource_probability_density']
...
def wind_battery_pem_tank_turb_optimize(time_points, h2_price=h2_price_per_kg, verbose=False, plot=False):
    from timeit import default_timer
    start = default_timer()
    # create the multiperiod model object
    mp_model = MultiPeriodModel(n_time_points=time_points,
    ...
    mp_model.build_multi_period_model(cap_factors)
    ...
    for blk in blks:
        if NPV_obj:
            ...
            if h2_contract:
                ...
                blk.hydrogen_revenue = Expression(expr=m.h2_price_per_kg / h2_mols_per_kg * (
                    blk.fs.h2_tank.outlet_to_pipeline.flow_mol[0] - blk.fs.mixer.purchased_hydrogen_feed_state[0].flow_mol) * 3600)
    ...
    h2_tank_out.append([pyo.value((blks[i].fs.h2_tank.outlet_to_pipeline.flow_mol[0] + blks[i].fs.h2_tank.outlet_to_tank.flow_mol[0]) * 3600 / 500) for i in range(time_points)])
    h2_tank_holdup.append(
        [pyo.value(blks[i].fs.h2_tank.tank_holdup[0]) for i in range(time_points)])
    h2_purchased.append([pyo.value(blks[i].fs.mixer.purchased_hydrogen_feed_state[0].flow_mol) * 3600 / h2_mols_per_kg for i in range(time_points)])
    ...
    h2_turbine_elec.append([pyo.value(blks[i].fs.h2_turbine.electricity[0]) for i in range(time_points)])
    turb_kwh.append(
        [pyo.value(blks[i].fs.h2_turbine.turbine.work_mechanical[0]) * -1e-3 for i in range(time_points)])
    comp_kwh.append(
        [pyo.value(blks[i].fs.h2_turbine.compressor.work_mechanical[0]) * 1e-3 for i in range(time_points)])
    elec_income.append([pyo.value(blks[i].profit) for i in range(time_points)])
...
if __name__ == "__main__":
    wind_battery_pem_tank_turb_optimize(time_points=7 * 24, h2_price=h2_price_per_kg, verbose=False, plot=True)
