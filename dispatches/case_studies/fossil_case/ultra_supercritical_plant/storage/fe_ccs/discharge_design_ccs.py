...
from idaes.models.unit_models import HeatExchanger, Heater, MomentumMixingType
...
def create_discharge_model(m, add_efficiency=None, power_max=None):
    ...
    m.fs.discharge.iplp_source_disjunct = Disjunct(
        rule=iplp_source_disjunct_equations)
    m.fs.discharge.hxd_source_disjunct = Disjunct(
        rule=hxd_source_disjunct_equations)
...
def _make_constraints(m, add_efficiency=None, power_max=None):
    ...
    m.fs.discharge_power_out = pyo.Var(
        initialize=30,
        bounds=(0, 200),
        doc="Coal heat duty supplied to boiler in MW")

    m.fs.net_power = pyo.Expression(
        expr=(m.fs.plant_power_out[0] + m.fs.discharge_power_out)
    )
...
def add_disjunction(m):
    ...
    # Add disjunction 2 for ccs steam source selection
    m.fs.ccs_source_disjunction = Disjunction(
        expr=[
...
def iplp_source_disjunct_equations(disj):
    ...
    # Declare turbine for storage system
    m.fs.discharge.iplp_source_disjunct.es_turbine = HelmTurbineStage(
        property_package=m.fs.prop_water
    )

    m.fs.discharge.iplp_source_disjunct.es_turbine.efficiency_isentropic.fix(0.8)

    for est in [m.fs.discharge.iplp_source_disjunct.es_turbine.control_volume]:
        iscale.set_scaling_factor(est.work, 1e-6)

              # + (m.fs.discharge.es_turbine.control_volume.work[0] * (-1e-6))
    # Add a constraint to storage turbine to ensure that the outlet
    # temperature is at the saturation temperature
    @m.fs.discharge.iplp_source_disjunct.es_turbine.Constraint(
        m.fs.time,
        doc="Turbine outlet should be a saturated steam")
    def constraint_esturbine_temperature_out(b, t):
        return (
    ...
    m.fs.discharge.iplp_source_disjunct.hxd_to_esturbine = Arc(
        source=m.fs.discharge.hxd.tube_outlet,
        destination=m.fs.discharge.iplp_source_disjunct.es_turbine.inlet,
        doc="Connection from HXD to ES turbine"
    )
...
def set_scaling_factors(m):
    ...
    iscale.set_scaling_factor(
        m.fs.ccs_reboiler.control_volume.heat, 1e-6)
...
def add_bounds(m, power_max=None):
    ...
    # m.fs.turbine_splitter[6].split_fraction[0.0, "outlet_3"].setlb(0)
    # m.fs.turbine_splitter[6].split_fraction[0.0, "outlet_3"].setub(1)
    ...
    #     unit_k.ratioP[:].setub(1e12)

    for unit_k in [m.fs.ccs_reboiler]:
        unit_k.inlet.flow_mol[:].setlb(0)
        unit_k.inlet.flow_mol[:].setub(m.flow_max)
        unit_k.outlet.flow_mol[:].setlb(0)
        unit_k.outlet.flow_mol[:].setub(m.flow_max)
    ...
    #     split2.inlet.flow_mol[:].setub(m.storage_flow_max)
    #     split2.to_ccs.flow_mol[:].setlb(0)
    #     split2.to_ccs.flow_mol[:].setub(m.storage_flow_max)
    #     split2.to_esturb.flow_mol[:].setlb(0)
    #     split2.to_esturb.flow_mol[:].setub(m.storage_flow_max)
    #     split2.split_fraction[0.0, "to_ccs"].setlb(0)
    #     split2.split_fraction[0.0, "to_ccs"].setub(1)
    #     split2.split_fraction[0.0, "to_esturb"].setlb(0)
    #     split2.split_fraction[0.0, "to_esturb"].setub(1)
...
def print_model(_, nlp_model, nlp_data):
    ...
    print('    ___________________________________________')
    print('     Disjunction 2:')
    if nlp.iplp_source_disjunct.binary_indicator_var.value == 1:
        print('      CCS Steam from IP LP crossover is selected')
    elif nlp.hxd_source_disjunct.binary_indicator_var.value == 1:
        print('      CCS Steam from HXD is selected')
    else:
        print('      Error: There are no more alternatives')
    print('    ___________________________________________')
    print()
...
def add_flugas_calculations(m):
    m.fs.co2_mw = Param(
        initialize=44.01,
        doc='molar weight of CO2 in g/mol'
    )
    m.fs.SR = Param(
        initialize=1.2,
        doc='Stoichiometric ratio used to calculate coal flow rate'
    )
    m.fs.coal_LHV = Param(
        initialize=29544,
        doc='LHV for Illinois no. 6 Bituminous coal from Baseline Rev 4, J/g'
    )
    m.fs.carbon_wt = Param(
        initialize=0.4972,
        doc='Carbon in dry weight basis of the coal from Baseline Rev 4'
    )
    m.fs.co2_molefrac = Param(
        initialize=0.14,
        doc='Mole fraction of CO2 in the flue gas of USC, 1999 DOE report'
    )
    m.fs.fg_flow_mol = Var(
        m.fs.config.time,
        initialize=1000,
        bounds=(0, 1e6),
        doc='Molar flow rate of flue gas')

    @m.fs.Constraint(m.fs.time,
                     doc="Mole flow of flue gas constraint")
    def eq_fluegas_flow(b, t):
        return (
            m.fs.fg_flow_mol[t] * (m.fs.co2_mw *
                                   m.fs.coal_LHV *
                                   m.fs.co2_molefrac) ==
            (m.fs.carbon_wt * m.fs.SR * (m.fs.reheater[2].heat_duty[t] +
                                          m.fs.reheater[1].heat_duty[t] +
                                          m.fs.boiler.heat_duty[t]))
            )
    
    return m
    
def add_capture_calculations(m):

    m.fs.fg_to_ccs_splitfraction = Var(
        m.fs.config.time,
        initialize=0.5,
        bounds=(0, 1),
        doc='Split of total flue gas sent to CCS')
    
    # m.fs.co2_captured = Var(
    #     m.fs.config.time,
    #     initialize=500,
    #     bounds=(0, 1e5),
    #     doc='Weight of CO2 captured CCS')

    # A specific reboiler duty of 3 GJ/tonne of CO2 captured is assumed for the
    # CCS unit performing at 95% capture rate and using an advanced solvent
    # such as Gen2
    # Units: 3 GJ/tonne = 3000 J/g
    m.fs.SRD = Param(
        initialize=3000,
    ...
    # @m.fs.Constraint(m.fs.time,
    #                  doc="Weight of CO2 captured")
    # def eq_min_co2_captured(b, t):
    #     return (
    #         m.fs.fg_to_ccs_splitfraction[t] >= 0.5)

    @m.fs.Constraint(m.fs.time,
                     doc="Reboiler heat duty constraint")
...
if __name__ == "__main__":
    ...
    # Add boiler fireside
    m_usc = add_flugas_calculations(m_usc)
    m_usc = add_capture_calculations(m_usc)
