...
def submit_job(
    ...
    file_name = os.path.join(job_scripts_dir, f"benchmark_wind_battery_parameterized_sim_rf_{int(reserve_factor * 1e2)}_shortfall_{shortfall}.sh")
    with open(file_name, "w") as f:
        f.write(
            ...
            + "#$ -m ae\n"
            + "#$ -q long\n"
            + f"#$ -N re-wind-parameterized-battery-sim_{sim_id}\n"
