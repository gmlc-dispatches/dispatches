...
# input layer 8 nodes, output 32 nodes for frequency of each representative day.

# attention: Now only work for 'only_dispatch' case. Have not considered wind+pv data into the NN yet. 
...
def main():
	this_file_path = os.getcwd()
	inp_file = os.path.join(this_file_path, '..\\datasets\\prescient_generator_inputs.h5')
	pred_csv = os.path.join(this_file_path, '..\\datasets\\Dispatch_shuffled_data_0.csv')
	x = read_input_x(inp_file, pred_csv)

	clustering_model_path = os.path.join(this_file_path, '..\\clustering_results\\result_6400years_shuffled_0_30clusters_OD.json')
	clustering_model = load_cluster_model(clustering_model_path)
