...
class TSA64K:
    def __init__(self, LMP_data, dispatch_data, method):
    ...
    def read_data(self):
        ...
        '''
        read clustering data from two csv files
        
        return: 
            two numpy array with lmp and dispatch data.
        '''

        df_lmp = pd.read_csv(self.LMP_data)
        df_dispatch = pd.read_csv(self.dispatch_data)

        # drop the first column
        df_lmp = df_lmp.iloc[: , 1:]
        df_dispatch = df_dispatch.iloc[: , 1:]

        # transfer the data to the np.array
        lmp_array = df_lmp.to_numpy(dtype = float)
        dispatch_array = df_dispatch.to_numpy(dtype = float)

        return lmp_array, dispatch_array

    def transform_data(self, lmp_array, dispatch_array):
        ...
        datasets = []
        time_len = 24
        day_num = int(np.round(len(lmp_array[0])/time_len))
        ...
        for i in range(day_num):
            day_data = np.vstack((lmp_year_0[i*time_len:(i+1)*time_len],dispatch_year_0[i*time_len:(i+1)*time_len]))
            datasets.append(day_data)
        datasets = np.array(datasets)
        # print(len(datasets))

        # filter the price spike
        delete_index = []
        for i, j in enumerate(datasets):
            # if we have price spike in certain day, delete that day from the dataset
            if np.max(j[0]) >= 200:
                delete_index.append(i)
            else:
                continue

        new_datasets = np.delete(datasets,delete_index, axis = 0).tolist()
        train_data = to_time_series_dataset(new_datasets)
        # print(day_num)
    ...
    def cluster_data(self, train_data, clusters):
        ...
        km.to_json('C:\\Users\\JKLCh\\Desktop\\ND_PhD\\Research\\clustering\\tslearn\\filter_ps\\'+str(clusters)+'_clusters.json')
